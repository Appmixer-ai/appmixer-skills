 ...
 * connectorsDir (for coverage validators) comes from $VERO_CONNECTORS_DIR/src,
...
if (!target || target === '-h' || target === '--help') {
    console.log('Usage: node validate.js <flow.json | flows-dir>   (connectorsDir from $VERO_CONNECTORS_DIR/src)');
...
const connectorsDir = process.env.VERO_CONNECTORS_DIR
    ? path.join(process.env.VERO_CONNECTORS_DIR, 'src')
    : deriveConnectorsSrc(path.resolve(target));

// Resolve the list of flow files: a single .json file, or every
// test-flow-*.json in a directory.
let flowPaths;
if (stat.isDirectory()) {
    flowPaths = fs.readdirSync(target)
        .filter((f) => f.startsWith('test-flow-') && f.endsWith('.json'))
        .sort()
        .map((f) => path.join(target, f));
    if (flowPaths.length === 0) {
        console.error(`No test-flow-*.json files found in ${target}`);
        process.exit(1);
    }
} else if (target.endsWith('.json')) {
    flowPaths = [target];
} else {
    console.error(`Not a JSON file or directory: ${target}`);
    process.exit(1);
}
