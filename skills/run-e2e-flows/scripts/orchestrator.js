...
// Override with VERO_APPMIXER_UI_URL when neither convention holds.
function designerUrl(baseUrl, flowId) {
    try {
        const ui = process.env.VERO_APPMIXER_UI_URL
    ...
    while (!TERMINAL.has(state)) {
        ...
        switch (state) {
            ...
            case S.INIT: {
                ...
                // VERO_APPMIXER_ACCOUNT_ID pins a specific account when the service has several
                // (e.g. an old one without the required OAuth scopes next to a re-consented one).
                const { accountIds } = await api.reassignAccounts(ctx.flowId, ctx.connectorPrefixes, process.env.VERO_APPMIXER_ACCOUNT_ID || null);
            ...
            case S.TRIAGE: {
                ...
                // Timeout with ZERO errors = nothing failed, an external event just hasn't arrived
                // (webhook notification latency varies from seconds to many minutes). Re-run once
                // deterministically before handing it to the agent — there is nothing to "fix".
                if (String(ctx.result).startsWith('timeout') && ctx.errors.length === 0 && !ctx.timeoutRetried) {
            ...
            case S.DET_FIX: {
                ...
                const { assigned } = await api.reassignAccounts(ctx.flowId, ctx.connectorPrefixes, process.env.VERO_APPMIXER_ACCOUNT_ID || null);
