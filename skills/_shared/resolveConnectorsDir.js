 ...
 *
 * Order:
 *   1) $VERO_CONNECTORS_DIR (explicit, e.g. CI)
...
export function resolveConnectorsDir(start = process.cwd()) {
    if (process.env.VERO_CONNECTORS_DIR) {
        return path.resolve(process.env.VERO_CONNECTORS_DIR);
    }
    ...
    throw new Error(
        'Cannot resolve connectors dir: set VERO_CONNECTORS_DIR or run from inside an ' +
