...
All connector design knowledge lives in `$APPMIXER_SKILL_ROOT/_shared/instructions/` — bundled with this plugin, the single source of truth for connector standards (the Node-dependencies block above resolves `APPMIXER_SKILL_ROOT` and fetches the bundle when missing). Real-world example connectors: https://github.com/appmixer-ai/appmixer-connectors.
...
Auth is usually already set up. Follow the `test-components` skill to test only the
...
directly (init-connector, test-components, review-component-standards,
...
1. **Lint + repo validator** before publishing — catch errors early:
   ...
   # dynamic outPort sources, output examples, bundle bumps, …) — only when the
   # workspace ships this validator (the appmixer-connectors repo does)
...
`<APPMIXER_SKILL_CONNECTORS_DIR>/src/appmixer/<connector>/artifacts/ai-artifacts/pipeline-state.json`
