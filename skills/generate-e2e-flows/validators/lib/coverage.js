...
export const validateAssertFieldSpecificity = (flowJson, connectorsDir) => {
    ...
    for (const [compId, comp] of Object.entries(flowJson.flow)) {
        ...
        for (const [sourceId, sourceConfig] of Object.entries(transformIn(comp))) {
            ...
            // when the flow sets a single-item/per-record outputType — "first" or
            // "object" (microsoft-style) and "item" (xero-style, one message per
            // record). Its dynamic source then returns per-field options, so
            // $.comp.out.field IS valid. (Fan-out/empty-result hazards of the
            // per-record modes are the outputtype-fanout validator's job.)
            const sourceOutputType = getComponentOutputType(sourceComp);
            const emitsItemFields = ['first', 'object', 'item'].includes(sourceOutputType);
            ...
            const walk = (obj) => {
                ...
                if (obj.variable && typeof obj.variable === 'string') {
                    ...
                    // Array-mode wrappers ARE the declared field: "items" for outputType
                    // "items" (xero-style), "result" for "array" (microsoft-style) —
                    // asserting <port>.items/<port>.result notEmpty is the recommended
                    // pattern for connectors without "first".
                    const wrapperField = fieldMatch
                        && ((sourceOutputType === 'items' && fieldMatch[3] === 'items')
                            || (sourceOutputType === 'array' && fieldMatch[3] === 'result'));
                    if (fieldMatch && !wrapperField && fieldMatch[1] === sourceId && hasDynamicSource && !hasStaticOptions && !emitsItemFields) {
                        ...
                        // Suggest a fix from the outputType values the component ACTUALLY
                        // declares — connectors differ ("first" vs xero's item/items/file)
                        // and recommending an undeclared value renders as a designer error.
                        const declared = (schema.inPorts?.[0]?.inspector?.inputs?.outputType?.options || [])
                            .map((o) => o.value)
                            .filter(Boolean);
                        let suggestion;
                        if (declared.includes('first')) {
                            suggestion = `Set outputType "first" on "${sourceId}" so it emits a single item with ` +
                                `named fields, then assert $.${sourceId}.${fieldMatch[2]}.${fieldMatch[3]} directly.`;
                        } else if (declared.includes('items')) {
                            suggestion = `This component declares outputType [${declared.join(', ')}] (no "first"): use ` +
                                `"items" and assert $.${sourceId}.${fieldMatch[2]}.items notEmpty (always emits, even ` +
                                'empty), or keep a per-record mode only when the flow guarantees a non-empty result.';
                        } else if (declared.includes('object')) {
                            suggestion = `This component declares outputType [${declared.join(', ')}] (no "first"): use ` +
                                '"object" with maxRecords: 1 and guarantee a non-empty result (an empty per-record ' +
                                'output emits nothing and AfterAll times out).';
                        } else {
                            // No outputType input at all — a fixed-shape Get component. Its
                            // dynamic options may well declare the field; this cannot be
                            // verified statically.
                            suggestion = 'The component declares no outputType input (fixed-shape ' +
                                'output): the field may be offered by its dynamic options — verify ' +
                                'with validate-variables after upload; if it is not offered, ' +
                                'reference the port root and extract the leaf with g_jsonPath.';
                        }
