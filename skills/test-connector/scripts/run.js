...
local flow JSON per the E2E run reference (13-e2e-run.md) rules and re-runs this script — INIT re-uploads the
local file and rebinds accounts on every run.

Connector publish (appmixer pack/publish) is NOT done here — publish the connector first (per the
E2E upload reference, 12-e2e-upload.md, or manually) so its components are available on the instance.
