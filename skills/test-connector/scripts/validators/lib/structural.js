...
export const validateProcessE2EResults = (components) => {
    ...
    // the E2E upload step), so they need not be present in the committed flow JSON.
...
export const validateDynamicOutputAsserts = (components) => {
    ...
    for (const [compId, comp] of Object.entries(components)) {
        ...
        for (const [sourceId, sourceConfig] of Object.entries(transformIn(comp))) {
            ...
            // When the flow sets a single-item/per-record outputType on the source
            // ("first"/"object" microsoft-style, "item" xero-style), it emits items
            // with named fields — $.comp.out.field is valid, don't warn. (Empty-result
            // and fan-out hazards of per-record modes are outputtype-fanout's job.)
            ...
            if (['first', 'object', 'item'].includes(sourceOutputType)) continue;
            ...
            const walk = (obj) => {
                ...
                if (obj.variable && typeof obj.variable === 'string') {
                    ...
                    if (match && match[1] === sourceId) {
                        const port = match[2];
                        const field = match[3];
                        // Array-mode wrappers ARE the declared field: "items" for
                        // outputType "items" (xero-style), "result" for "array"
                        // (microsoft-style) — asserting <port>.items/<port>.result
                        // notEmpty is the recommended pattern, don't warn on it.
                        if ((sourceOutputType === 'items' && field === 'items')
                            || (sourceOutputType === 'array' && field === 'result')) return;
...
export const deterministicValidation = (flowJson, { hasComponentSchemas = false } = {}) => {
    const errors = [
    ...
    errors.push(
        ...
        // Heuristic only — when component.json schemas are available, the precise
        // component-aware version in coverage.js (assert-field-on-dynamic-output)
        // runs instead; running both just duplicates noise with false positives
        // on components that DO have static options.
        ...(hasComponentSchemas ? [] : validateDynamicOutputAsserts(components)),
    );
