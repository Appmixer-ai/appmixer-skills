...
description: Publish an Appmixer connector to a live instance and upload E2E test flows. Handles pack/publish, auth account creation, flow upload, account assignment, and validation. Use after generate-e2e-flows has produced flow JSONs, or when connector source changed. Triggers on "upload flows", "setup e2e", "deploy connector", "prepare e2e", "publish connector".
license: MIT
metadata:
  author: Appmixer
  version: "0.1.9"
...
- **Run from the connector workspace** — the current directory (or a parent)
  contains `src/appmixer/<connector>/`; the Node scripts resolve the workspace
  from the cwd. `APPMIXER_SKILL_CONNECTORS_DIR` is an optional override for
  running from elsewhere (see the worktree section below).
- Test flow JSON files in `artifacts/test-flows/` (generated by `generate-e2e-flows` skill)
...
test -d "${APPMIXER_SKILL_CONNECTORS_DIR:-.}/src/appmixer" || { echo "Not in a connector workspace: run from a dir containing src/appmixer (or set APPMIXER_SKILL_CONNECTORS_DIR)"; exit 1; }
```
...
cd src/appmixer   # from the workspace root
...
appmixer pack <connector>
appmixer publish "$PWD/appmixer.<connector>.zip"
```
...
sleep 1
appmixer publish "$PWD/appmixer.<vendor>.zip"
```
